// Spray GenX WRA Manager — UX Integrated 3
// Version: 2026.07.04 Integrated-3
// Purpose: Scriptable proposal/invoice manager with two-column UX, legacy import repair, backups, archive browsing, and small bottom-right document seal.
...
async function settingsMenu() {
  ...
  s.sealText = a.textFieldValue(4).trim() || s.companyName;
  s.sealSubtext = a.textFieldValue(5).trim();
...
function normalizeRecord(raw, filename, path) {
  ...
  const d = {
    ...
    contact: first(customerObj.contact, raw.contact, raw.gc, raw.GC),
    phone: first(customerObj.phone, raw.phone),
    email: first(customerObj.email, raw.email),
    title: first(job.title, raw.project, raw.Project, raw.title, raw.jobName, raw.job_title),
    site: first(job.site, job.address, customerObj.address, raw.site, raw.address, raw.location),
    city: first(job.city, customerObj.city, raw.city),
    category: first(job.category, raw.category),
    summary: first(scope.summary, raw.summary),
    details: first(scope.details, scope.description, raw.details, typeof raw.scope === "string" ? raw.scope : "", raw.description),
    notes: [scope.exclusions, scope.notes, raw.exclusions, raw.notes].filter(Boolean).join("\n\n"),
    total,
    deposit: paid,
    balance_due: balance,
...
function slim(d) {
  return {
    id: d.id, kind: d.kind, path: d.path || "", customer: d.customer || "", title: d.title || "", site: d.site || "", city: d.city || "",
    status: d.status || "open", total: Number(d.total || 0), deposit: Number(d.deposit || 0), balance_due: Number(d.balance_due || 0),
