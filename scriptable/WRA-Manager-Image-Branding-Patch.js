...
// Removes embedded medallion base64, restores image-file logo, and moves medallion left of pricing.
...
// Add Templates folder to DIRS if missing.
...
// Remove giant embedded medallion constant.
src = src.replace(/\n?const MEDALLION_IMAGE_BASE64\s*=\s*"[\s\S]*?";\s*\n/, "\n");

const newWriteHtml = String.raw`function writeHtml(d, kind) {
  const s = getSettings();
  ensureMedia(d);
  const outDir = kind === "invoice" ? DIRS.invoices : DIRS.proposals;
  const path = fm.joinPath(outDir, `${d.id}.html`);

  // Real branding image files — no embedded base64.
  const logoFile = materializeBrandImage(outDir, "logo", [
    "logo.png", "Logo-SprayGenxLLC.PNG", "Actual-Logo-SprayGenX.png",
    "SprayGenX-Logo.png", "SprayGenX-Logo.jpg", "SprayGenX Logo.png"
  ], "SprayGenX-Logo.png");
  const medallionFile = materializeBrandImage(outDir, "medallion", [
    "medallion.png", "medallion.jpg", "SprayGenX-Medallion.png", "SprayGenX-Medallion.jpg",
    "WRA-Medallion.png", "WRA-Medallion.jpg"
...
.page{background:white;max-width:820px;margin:0 auto;min-height:100vh;padding:28px 30px 74px;box-sizing:border-box;position:relative}
...
.brand{max-width:54%;display:flex;align-items:flex-start;gap:13px}.brand-logo{display:block;max-width:150px;max-height:72px;width:auto;height:auto;object-fit:contain}.brand-copy{padding-top:2px}.brand h1{margin:0;font-size:24px}.brand p,.customer p,.docline p{margin:3px 0;color:#444}
.customer{text-align:right;max-width:42%}.customer .label,.box h3{margin:0 0 7px;text-transform:uppercase;font-size:12px;letter-spacing:.08em;color:#444}.customer .name{font-size:19px;font-weight:800;color:#111}
...
.grid{display:grid;grid-template-columns:1fr 1fr;gap:14px}.box{border:1px solid #ddd;border-radius:9px;padding:12px 14px;margin-bottom:14px}.scope{white-space:pre-wrap}
.total-box{display:grid;grid-template-columns:112px 1fr;gap:14px;align-items:center;min-height:138px}.total-art{display:flex;align-items:center;justify-content:center}.total-copy{min-width:0}.price{font-size:30px;font-weight:800;text-align:right;margin:4px 0 14px}.total-copy p:not(.price){margin:8px 0;text-align:right}.seal{width:96px;height:96px;margin:0;opacity:.86}.seal img{display:block;width:100%;height:100%;object-fit:contain}
.terms{border-top:1px solid #ddd;margin-top:12px;padding-top:10px;font-size:12px}
@media screen and (max-width:650px){.top,.grid,.docline{display:block}.brand,.customer{max-width:none}.brand{display:flex}.customer{text-align:left;margin-top:14px}.page{padding:22px 22px 70px}.total-box{grid-template-columns:100px 1fr}}
...
function materializeBrandImage(outDir, type, candidates, outputName) {
  ...
  for (const dir of roots) {
    ...
    for (const name of candidates) {
      ...
      if (fm.fileExists(p)) { src = p; break; }
    }
    if (src) break;
    try {
      const hit = fm.listContents(dir).find(n => {
        const low = n.toLowerCase();
        return /\.(png|jpe?g)$/i.test(n) && low.includes(type);
      });
      if (hit) src = fm.joinPath(dir, hit);
    } catch (e) {}
    if (src) break;
  }
  if (!src) return "";
  try {
    downloadIfNeeded(src);
...
// Replace writeHtml plus any old materializeMedallion helper section.
...
// Remove legacy medallion materializer if it still exists later in the file.
...
a.message = "Done. Main logo now uses an image file. Medallion uses an image file and sits left of the pricing.\n\nBackup: " + backup;
a.addAction("OK");
await a.presentAlert();
Script.complete();
