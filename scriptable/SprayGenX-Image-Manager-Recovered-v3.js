// Spray GenX Image Manager — v3.3
// Auto-attaches uploads to image blocks, adds upload progress notifications,
// visual image previews, and block deletion controls.
...
async function progressPing(done,total,project){
  ...
  try{
    ...
    n.title=`Spray GenX Upload: ${done}/${total}`;
    n.body=`${project}: ${done} image${done===1?"":"s"} uploaded.`;
    n.sound="default";
    await n.schedule();
  }catch(_){ }
}
...
function normalizeLibrary(l){
  ...
  l.blocks=l.blocks.map((b,n)=>{
    ...
    let locs=unique([...(Array.isArray(b.siteLocations)?b.siteLocations:[]),...Object.keys(b.views||{}).filter(k=>b.views[k])]);if(!locs.length)locs=DEFAULT_VIEWS.slice();
...
async function attachUploadedToBlock(projectName,paths){
  ...
  let projectSlug=slug(projectName),b=(LIB.json.blocks||[]).find(x=>x.slug===projectSlug||slug(x.title)===projectSlug);
  if(!b){
...
async function uploadImageObjects(images,sourceLabel="Photos"){
  images=(images||[]).filter(Boolean).slice(0,MAX_BATCH);if(!images.length){await alertMsg("No Photos","No photos were received.");return}
  let project=await ask("Upload Batch / Project","Example: industrial-floor-painting",today());if(project===null)return;project=cleanFolder(project);
  let run=stamp(),uploaded=[],fm=FileManager.local();
  for(let i=0;i<images.length;i++){
    let n=String(i+1).padStart(3,"0"),safe=`photo-${run}-${n}.jpg`,temp=fm.joinPath(fm.temporaryDirectory(),safe);fm.writeImage(temp,images[i]);
    let repoPath=`${INBOX_ROOT}/${project}/${safe}`;await putBinary(repoPath,fm.read(temp),`Inbox upload: ${safe}`);uploaded.push(repoPath);try{fm.remove(temp)}catch(_){ }
...
async function uploadMultipleFiles(){
  ...
  if(!Array.isArray(paths))paths=paths?[paths]:[];paths=unique(paths).slice(0,MAX_BATCH);if(!paths.length){await alertMsg("No Files","No files selected.");return}
  ...
  for(let i=0;i<paths.length;i++){
    ...
    await putBinary(repoPath,fm.read(p),`Inbox upload: ${safe}`);uploaded.push(repoPath);await progressPing(i+1,paths.length,titleCase(project));
  }
  let b=await attachUploadedToBlock(project,uploaded);
  await alertMsg("Batch Upload Complete",`${uploaded.length} file(s) uploaded.\n\n${b?`Added to image block: ${b.title}`:"Images uploaded, but library attachment could not be completed."}`);
}
async function uploadMenu(){
  let shared=Array.isArray(args.images)&&args.images.length,items=[];if(shared)items.push({label:`Import ${Math.min(args.images.length,MAX_BATCH)} Shared Photos Now`,id:"shared"});
  ...
  let a=await choose("Upload Images",items,"Uploads now attach directly to the matching image block.");if(!a)return;
  if(a.id==="shared")await uploadSharedPhotos();if(a.id==="sharehelp")await alertMsg("Multiple Photos","Photos → Select → choose photos → Share → Scriptable → this Image Manager.");if(a.id==="files")await uploadMultipleFiles();if(a.id==="one")await uploadOnePhoto();
}

async function pickCategory(multi=false,current=[]){
  let cats=LIB.json.categories||[],chosen=[];while(true){let item=await choose(multi?"Add Category":"Choose Category",cats.map(c=>({label:(current.includes(c.id)||chosen.includes(c.id)?"✓ ":"")+c.label,id:c.id})));if(!item)break;chosen.push(item.id);if(!multi)break;if(!await confirm("Add Another Category?","Add another category to this same block?","Add More"))break}
...
async function editBlock(existing=null){
  ...
  let st=await choose("Status",["draft","published","hidden","archived"].map(x=>({label:x,id:x})),`Current: ${b.status}`);if(st)b.status=st.id;b.visible=b.status!=="hidden";b.id=b.id||`${slug(b.title)}-${stamp()}`;b.slug=slug(b.title);b.cover=b.cover||(b.images[0]&&b.images[0].path)||"";return b
}
...
async function viewBlockImages(b){
  ...
  let cards=images.map((im,i)=>{let p=imgPath(im),caption=im.caption||im.alt||"",cover=p===b.cover||im.role==="cover";return `<figure><a href="${esc(rawUrl(p))}"><img src="${esc(rawUrl(p))}" loading="lazy"></a><figcaption><strong>${i+1}${cover?" · COVER":""}</strong>${caption?`<br>${esc(caption)}`:""}</figcaption></figure>`}).join("");
  let html=`<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><style>body{margin:0;background:#0d1824;color:#fff;font-family:-apple-system;padding:16px}h1{font-size:22px;margin:0 0 6px}.sub{opacity:.65;margin-bottom:16px}.grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}figure{margin:0;background:#142638;border-radius:12px;overflow:hidden}img{display:block;width:100%;aspect-ratio:1/1;object-fit:cover;background:#09111a}figcaption{padding:9px 10px;font-size:12px;line-height:1.35}@media(min-width:700px){.grid{grid-template-columns:repeat(3,minmax(0,1fr))}}</style></head><body><h1>${esc(b.title)}</h1><div class="sub">${images.length} image${images.length===1?"":"s"} · tap any image for full size</div><div class="grid">${cards}</div></body></html>`;
  let w=new WebView();await w.loadHTML(html);await w.present(true)
}

async function deleteBlock(b){
  let ok=await confirm("Delete Image Block?",`${b.title}\n\nThis removes the block from image-library.json. It does NOT delete the underlying image files from GitHub.`,`Delete Block`);if(!ok)return false;
  LIB.json.blocks=(LIB.json.blocks||[]).filter(x=>x.id!==b.id);await saveLibrary(`Delete image block: ${b.title}`);await alertMsg("Deleted",`${b.title} was removed from the image library.`);return true
}

async function chooseBlockOrDelete(){
  let blocks=LIB.json.blocks||[],items=[];
  blocks.forEach((b,i)=>{items.push({label:`📁 ${b.title} — ${b.images.length} img`,type:"open",i});items.push({label:`🗑 Delete: ${b.title}`,type:"delete",i})});
  return await choose("Image Blocks",items,"Each block has a delete option directly beneath it.")
}

async function manageBlock(){
  let pick=await chooseBlockOrDelete();if(!pick)return;let b=(LIB.json.blocks||[])[pick.i];if(!b)return;
  if(pick.type==="delete"){await deleteBlock(b);return}
  let act=await choose(b.title,[{label:"View Images",id:"view"},{label:"Edit Block",id:"edit"},{label:"Hide / Show",id:"toggle"},{label:"🗑 Delete Block",id:"delete"},{label:"Copy Block JSON",id:"copy"},{label:"Health",id:"health"}]);if(!act)return;
  if(act.id==="view")await viewBlockImages(b);
  if(act.id==="edit"){let nb=await editBlock(b);if(!nb)return;Object.assign(b,nb);await saveLibrary(`Studio edit image block: ${b.title}`)}
  ...
  if(act.id==="copy"){Pasteboard.copy(JSON.stringify(b,null,2));await alertMsg("Copied","Block JSON copied to clipboard.")}
...
async function publishConverted(){
  ...
  let used=usedPaths(),avail=convertedImages(cf.json).filter(x=>!used.has(x.converted));if(!avail.length){await alertMsg("Nothing New","No unpublished converted images found.");return}
  let groups={};avail.forEach(x=>{let f=folderOf(x.converted);(groups[f]=groups[f]||[]).push(x)});let keys=Object.keys(groups).sort().reverse();let pick=await choose("Converted Batch",keys.map(k=>({label:`${k} — ${groups[k].length}`,id:k})).concat([{label:`All unpublished — ${avail.length}`,id:"__all"}]));if(!pick)return;
  let batch=pick.id==="__all"?avail:groups[pick.id],title=await ask("Block Title",titleCase(pick.id==="__all"?folderOf(batch[0].converted):pick.id),titleCase(pick.id==="__all"?folderOf(batch[0].converted):pick.id));if(!title)return;
...
async function main(){
  ...
  while(true){
    let a=await choose("Spray GenX Image Manager v3.3",[{label:"Upload Images",id:"upload"},{label:"Publish Converted Images",id:"converted"},{label:"+ New Image Block",id:"new"},{label:"Edit / View Image Blocks",id:"manage"},{label:"Health Check",id:"health"},{label:"Counts",id:"counts"},{label:"Export JSON",id:"export"},{label:"Reload Library",id:"reload"},{label:"Done",id:"done"}]);
    if(!a||a.id==="done")break;
    if(a.id==="upload"){await uploadMenu();continue}if(a.id==="reload"){LIB=null;await ensureLoaded();continue}if(!await ensureLoaded())continue;
