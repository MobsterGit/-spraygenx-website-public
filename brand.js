...
document.querySelectorAll('.footer-grid').forEach(grid=>{const copy=grid.children&&grid.children[1];if(copy){copy.innerHTML='© <span data-year>'+new Date().getFullYear()+'</span> Spray GenX LLC. Professional Painting • Refinishing • Commercial Coatings <span class="footer-inline-links"><span>•</span><a href="gallery.html">Portfolio</a><span>•</span><a href="photo-library.html">Image Library</a></span>';}});
...
(function(){const grid=document.getElementById('homeRecentProjects');if(!grid)return;function esc(v){return String(v||'').replace(/[&<>"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c]));}function imgPath(img){return typeof img==='string'?img:(img&&img.path?img.path:'');}function firstImage(project){const imgs=Array.isArray(project.images)?project.images:[];const visible=imgs.find(img=>typeof img==='string'||img.visible!==false);return project.cover||imgPath(visible)||'';}function short(v){const s=String(v||'Completed Spray GenX project.').trim();return s.length>74?s.slice(0,71).replace(/\s+\S*$/,'')+'…':s;}fetch('data/portfolio.json?ts='+Date.now()).then(r=>r.ok?r.json():null).then(data=>{const projects=(data&&Array.isArray(data.projects)?data.projects:[]).filter(p=>p.visible!==false&&firstImage(p)).slice().reverse().slice(0,4);if(!projects.length)return;grid.innerHTML=projects.map(p=>`<a class="project-card" href="gallery.html?project=${encodeURIComponent(p.title||'Spray GenX Project')}"><img src="${esc(firstImage(p))}" alt="${esc(p.title||'Spray GenX project')}"><h3>${esc(p.title||'Spray GenX Project')}</h3><p>${esc(short(p.description))}</p></a>`).join('');}).catch(()=>{});})();
(function(){const target=new URLSearchParams(location.search).get('project');if(!target||!document.getElementById('portfolioSections'))return;const norm=v=>String(v||'').toLowerCase().replace(/\s+/g,' ').trim();let tries=0;const timer=setInterval(()=>{tries++;const cards=[...document.querySelectorAll('.portfolio-card')];const card=cards.find(c=>norm(c.querySelector('h3')?.textContent)===norm(target));if(card){clearInterval(timer);card.classList.add('project-jump-target');card.scrollIntoView({behavior:prefersReduced?'auto':'smooth',block:'center'});setTimeout(()=>card.classList.remove('project-jump-target'),2600);}else if(tries>80){clearInterval(timer);}},100);})();
