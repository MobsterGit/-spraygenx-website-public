...
// Version: 2026.08.09 Canonical-3
// Real image files only. Images folder + WebView preview.
...
async function home() {
  ...
  const r = new UITableRow(); r.height = 48; r.addText("Close"); t.addRow(r);
  await t.present();
}
function pair(t,a,fa,b,fb){const r=new UITableRow();r.height=62;const l=r.addButton(a);l.widthWeight=50;l.onTap=fa;const q=r.addButton(b);q.widthWeight=50;q.onTap=fb;t.addRow(r)}
...
async function editDoc(d,kind){
  ...
  p.addTextField("Total price",String(d.total||""));p.addTextField(kind==="invoice"?"Paid":"Deposit",String(d.deposit||""));p.addTextField("Status",d.status||(kind==="invoice"?"unpaid":"open"));
  p.addAction("Next");p.addCancelAction("Cancel"); if(await p.presentAlert()===-1)return null;
  d.total=num(p.textFieldValue(0));d.deposit=num(p.textFieldValue(1));d.balance_due=Math.max(0,d.total-d.deposit);d.status=clean(p.textFieldValue(2))||(kind==="invoice"?"unpaid":"open");

  d.summary=await textStep("Scope Summary",d.summary);d.details=await textStep("Scope Details",d.details);d.notes=await textStep("Notes / Exclusions",d.notes);
  await projectImageFlow(d); d.updated=today(); sortKeys(d); return d;
}
...
async function projectImageFlow(d){
  ensureMedia(d);const a=new Alert();a.title="Project Image";a.message=hasProjectImage(d)?"Keep, replace, or remove the project image.":"Optional: add a project image.";
  ...
  if(c===1||c===2){try{const img=c===1?await Photos.fromLibrary():await Photos.fromCamera();const path=fm.joinPath(DIRS.photos,`${d.id}-${slug(d.title||d.customer||"project")}.jpg`);fm.writeImage(path,img);d.media.project_image_path=path}catch(e){await notice("Image Not Added",e)}}
...
async function afterSave(d,kind){
  ...
  a.addAction("Preview");a.addAction("Edit Again");if(kind==="proposal")a.addAction("Convert to Invoice");a.addAction("Done");
  const c=await a.presentSheet();if(c===0)await WebView.loadFile(writeHtml(d,kind));if(c===1)await docFlow(d,kind);if(kind==="proposal"&&c===2)await convertToInvoice(d);
}
...
async function documentTable(title,docs){
  ...
  docs.forEach(m=>{const d=loadDoc(m);const r=new UITableRow();r.height=84;const l=r.addText(`${d.kind==="invoice"?"INVOICE":"PROPOSAL"} ${d.id}`,[d.customer,d.title,d.status,d.site||d.city].filter(Boolean).join(" | "));l.widthWeight=75;const q=r.addText(money(d.total),d.updated||d.created||"");q.rightAligned();q.widthWeight=30;r.onSelect=async()=>openDoc(d);t.addRow(r)});
  await t.present();
}
async function openDoc(d){
  const kind=d.kind==="invoice"?"invoice":"proposal",a=new Alert();a.title=`${kind.toUpperCase()} ${d.id}`;a.message=`${d.customer||"No customer"}\n${d.title||"No title"}\n${money(d.total)}`;
  a.addAction("Preview");a.addAction("Edit");a.addAction("Project Image");a.addAction("Regenerate HTML");if(kind==="proposal")a.addAction("Convert to Invoice");a.addAction("Archive");a.addCancelAction("Back");
  const c=await a.presentSheet();if(c===0)await WebView.loadFile(writeHtml(d,kind));if(c===1)await docFlow(d,kind);if(c===2){await projectImageFlow(d);saveDoc(d,kind);writeHtml(d,kind)}if(c===3){writeHtml(d,kind);await notice("HTML Regenerated",d.id)}if(kind==="proposal"&&c===4)await convertToInvoice(d);
...
async function convertToInvoice(p){
  ...
  p.status="converted_to_invoice";p.updated=today();saveDoc(p,"proposal");saveDoc(inv,"invoice");writeHtml(p,"proposal");writeHtml(inv,"invoice");bumpNumber(inv.id,"invoice");await notice("Invoice Created",`${inv.id} from ${p.id}`);
}
...
async function settingsMenu(){
  const s=getSettings(),a=new Alert();a.title="Settings";["Company","Phone","Email","Service area","Tagline"].forEach((x,i)=>a.addTextField(x,[s.companyName,s.phone,s.email,s.serviceArea,s.tagline][i]||""));
...
function writeHtml(d,kind){
  const s=getSettings();ensureMedia(d);seedBrandImages();
  const outDir=kind==="invoice"?DIRS.invoices:DIRS.proposals,path=fm.joinPath(outDir,`${d.id}.html`);
  const logoFile=copyBrandToOutput("logo",outDir),medFile=copyBrandToOutput("medallion",outDir);
  const logo=logoFile?`<img class="brand-logo" src="${esc(logoFile)}" alt="Spray GenX LLC logo">`:"";
  const seal=medFile?`<div class="seal"><img src="${esc(medFile)}" alt="Spray GenX medallion"></div>`:"";
...
${projectPhotoHtml(d,outDir)}<section class="box"><h3>Scope Summary</h3><p class="scope">${esc(d.summary)}</p></section><section class="box"><h3>Scope Details</h3><p class="scope">${esc(d.details)}</p></section>
...
<section class="terms"><p><strong>Terms:</strong> ${esc(s.defaultTerms)}</p><p><strong>Warranty:</strong> ${esc(s.warrantyNote)}</p></section></main></body></html>`;
  fm.writeString(path,html);return path;
}
function projectPhotoHtml(d,outDir){const f=copyProjectToOutput(d,outDir);return f?`<section class="project-photo"><img src="${esc(f)}" alt="Project photo"></section>`:""}
function copyProjectToOutput(d,outDir){ensureMedia(d);const p=d.media.project_image_path;if(!p||!fm.fileExists(p))return"";try{downloadIfNeeded(p);const ext=/\.png$/i.test(p)?".png":".jpg",n=`${d.id}-project-image${ext}`,dst=fm.joinPath(outDir,n);replaceCopy(p,dst);return n}catch(e){return""}}

function seedBrandImages(){
  ensure(DIRS.images);
  seedOne("logo",["SprayGenX-Logo.png","SprayGenX-Logo.jpg","Actual-Logo-SprayGenX.png","Logo-SprayGenxLLC.PNG","logo.png","SprayGenX Logo.png"]);
  seedOne("medallion",["SprayGenX-Medallion.png","SprayGenX-Medallion.jpg","WRA-Medallion.png","WRA-Medallion.jpg","medallion.png","medallion.jpg"]);
}
function seedOne(type,names){
  const existing=findImage(DIRS.images,type,names);if(existing)return existing;
  for(const dir of [DIRS.templates,DIRS.photos,DIRS.proposals,DIRS.invoices,DIRS.root]){
    const src=findImage(dir,type,names);if(!src)continue;
    try{downloadIfNeeded(src);const ext=/\.jpe?g$/i.test(src)?".jpg":".png",dst=fm.joinPath(DIRS.images,type==="logo"?`SprayGenX-Logo${ext}`:`SprayGenX-Medallion${ext}`);replaceCopy(src,dst);return dst}catch(e){}
  }
  return"";
}
function findImage(dir,type,names){
  if(!dir||!fm.fileExists(dir))return"";
  for(const n of names){const p=fm.joinPath(dir,n);if(fm.fileExists(p))return p}
  try{const hit=fm.listContents(dir).find(n=>/\.(png|jpe?g)$/i.test(n)&&n.toLowerCase().includes(type));return hit?fm.joinPath(dir,hit):""}catch(e){return""}
}
function copyBrandToOutput(type,outDir){
  const names=type==="logo"?["SprayGenX-Logo.png","SprayGenX-Logo.jpg"]:["SprayGenX-Medallion.png","SprayGenX-Medallion.jpg"];
  const src=findImage(DIRS.images,type,names);if(!src)return"";
  try{downloadIfNeeded(src);const ext=/\.jpe?g$/i.test(src)?".jpg":".png",n=type==="logo"?`SprayGenX-Logo${ext}`:`SprayGenX-Medallion${ext}`,dst=fm.joinPath(outDir,n);replaceCopy(src,dst);return n}catch(e){return""}
}
...
function normalizeLegacy(raw,filename,path,forced){
  ...
  const id=first(raw.id,raw.docNo,raw.DocNo,raw.number,manager.invoice_id,manager.proposal_id,idFromFilename(filename));
  if(!id)return null;
  ...
  const d={id,kind,path,customer:first(cust.name,cust.client,raw.client,raw.Client,raw.customerName,typeof raw.customer==="string"?raw.customer:"",raw.name),contact:first(cust.contact,raw.contact,raw.gc,raw.GC),phone:first(cust.phone,raw.phone),email:first(cust.email,raw.email),title:first(job.title,raw.project,raw.Project,raw.title,raw.jobName,raw.job_title),site:first(job.site,job.address,cust.address,raw.site,raw.address,raw.location),city:first(job.city,cust.city,raw.city),category:first(job.category,raw.category),summary:first(scope.summary,raw.summary),details:first(scope.details,scope.description,raw.details,typeof raw.scope==="string"?raw.scope:"",raw.description),notes:[scope.exclusions,scope.notes,raw.exclusions,raw.notes].filter(Boolean).join("\n\n"),media:{project_image_path:first(raw.media&&raw.media.project_image_path,raw.project_image_path,raw.projectImagePath)},total,deposit:paid,balance_due:num(first(pricing.balance_due,raw.balance_due,raw.balanceDue,manager.balance_due,total-paid)),status:first(manager.status,manager.payment_status,raw.status,kind==="invoice"?"unpaid":"open"),created:toIso(first(manager.created_date,raw.date,raw.Date,raw.created,raw.createdDate,raw.created_at,today())),updated:toIso(first(manager.updated_date,raw.updated,raw.updatedDate,raw.updated_at,raw.created,raw.date,today()))};
  sortKeys(d);return d;
}
