// Spray GenX Image Manager — Recovered Master v3.2
// Adds Share Sheet batch import, Files multi-select, and actual image gallery previews.
...
async function uploadSharedPhotos(){
  ...
  if(!images.length){await alertMsg("No Shared Photos","Open Photos → Select → choose multiple photos → Share → Scriptable → this Image Manager.");return false}
  await uploadImageObjects(images,`Share Sheet (${images.length} selected)`);return true;
}
async function uploadMultipleFiles(){
  let folder=await ask("Inbox Folder Name","Example: westfield-control-room-floor",today());if(folder===null)return;folder=cleanFolder(folder);
  ...
  for(let i=0;i<paths.length;i++){
    let p=paths[i],n=String(i+1).padStart(3,"0"),safe=`${slug(base(p))}-${run}-${n}.${ext(p)}`,repoPath=`${INBOX_ROOT}/${folder}/${safe}`;
    await putBinary(repoPath,fm.read(p),`Inbox upload: ${safe}`);uploaded.push(repoPath);
  }
...
async function uploadMenu(){
  ...
  let a=await choose("Upload Images",items,"For multiple Photos, select them in Apple Photos first and Share them to this script.");if(!a)return;
  ...
  if(a.id==="files")await uploadMultipleFiles();if(a.id==="one")await uploadOnePhoto();
}
...
function normalizeLibrary(l){
  ...
  l.blocks=l.blocks.map((b,n)=>{
    let cats=Array.isArray(b.categories)?b.categories.filter(Boolean):[],cat=b.category||cats[0]||(l.categories[0]&&l.categories[0].id)||"uncategorized";cats=unique([cat,...cats]);
    let views=b.views||{},locs=unique([...(Array.isArray(b.siteLocations)?b.siteLocations:[]),...Object.keys(views).filter(k=>views[k])]);if(!locs.length)locs=DEFAULT_VIEWS.slice();
...
async function chooseBlock(){let blocks=LIB.json.blocks||[],item=await choose("Image Blocks",blocks.map((b,i)=>({label:`${b.title} — ${categoryLabel(b.category)} — ${b.images.length} img`,i})));return item?blocks[item.i]:null}
...
async function viewBlockImages(b){
  ...
  let cards=images.map((im,i)=>{
    let p=imgPath(im),caption=im.caption||im.alt||"",cover=p===b.cover||im.role==="cover";
    return `<figure><a href="${esc(rawUrl(p))}"><img src="${esc(rawUrl(p))}" loading="lazy"></a><figcaption><strong>${i+1}${cover?" · COVER":""}</strong>${caption?`<br>${esc(caption)}`:""}</figcaption></figure>`;
  }).join("");
...
async function manageBlock(){
  let b=await chooseBlock();if(!b)return;
  let act=await choose(b.title,["Edit Block","View Images","Hide / Show","Copy Block JSON","Health"].map(x=>({label:x,id:x})));if(!act)return;
  ...
  if(act.id==="View Images")await viewBlockImages(b);
...
async function publishConverted(){
  ...
  let summary=await ask("Summary","Completed Spray GenX project.","Completed Spray GenX project."),tags=csv(await ask("Tags","painting, refinishing, completed work","painting, refinishing, completed work")),cat=await pickCategory(false,[]);
  let b={id:`${slug(title)}-${stamp()}`,slug:slug(title),title,summary:summary||"Completed Spray GenX project.",customer:"",location:"",date:today(),category:cat,categories:[cat],tags,status:"published",visible:true,weight:25,priority:25,views:{library:true,latest:true,search:true},siteLocations:DEFAULT_VIEWS.slice(),fallback:"latest",cover:batch[0].converted,images:batch.map((x,i)=>({path:x.converted,caption:"",alt:title,visible:true,role:i===0?"cover":""}))};
  LIB.json.blocks.unshift(b);await saveLibrary(`Studio publish converted block: ${title}`);await alertMsg("Published",`${title}\n${b.images.length} converted image(s) added.`);
}
...
async function main(){
  ...
  while(true){
    let a=await choose("Spray GenX Image Manager v3.2",[{label:"Upload Images",id:"upload"},{label:"Publish Converted Images",id:"converted"},{label:"+ New Image Block",id:"new"},{label:"Edit / View Image Blocks",id:"manage"},{label:"Health Check",id:"health"},{label:"Counts",id:"counts"},{label:"Export JSON",id:"export"},{label:"Reload Library",id:"reload"},{label:"Done",id:"done"}]);
    if(!a||a.id==="done")break;
    if(a.id==="upload"){await uploadMenu();continue}if(a.id==="reload"){LIB=null;PORT=null;await ensureLoaded();continue}if(!await ensureLoaded())continue;
